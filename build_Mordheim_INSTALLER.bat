...
echo ================================================
echo  MORDHEIM - BUILD WINDOWS INSTALLER
echo ================================================
...
echo.
echo [1/3] Generating Mordheim.exe...
call build_Mordheim_ONEFILE.bat --no-pause
if errorlevel 1 goto :build_error

echo.
echo [2/3] Reading project version...
for /f "delims=" %%V in ('python -c "import sys; sys.path.insert(0, 'src'); import mordheim_optimizer; print(mordheim_optimizer.__version__)"') do set "APP_VERSION=%%V"
if not defined APP_VERSION goto :version_error
echo Version: %APP_VERSION%

echo.
echo [3/3] Generating installer...
"%ISCC_EXE%" "/DMyAppVersion=%APP_VERSION%" "installer\Mordheim.iss"
...
echo.
echo Output:
echo %CD%\dist\installer\Mordheim-Optimizer-Setup-%APP_VERSION%.exe
echo.
...
:build_error
echo ERROR: Mordheim.exe could not be generated.
goto :failed
