...
def _knowledge_root() -> Path:
    ...
    candidates.extend((
        ...
        Path(sys.prefix) / "share" / "mordheim_optimizer" / "knowledge" / "bands",
    ))
...
def _band_skills(raw_band: dict) -> tuple[BandSkill, ...]:
    result = []
    seen = set()
    section_headings = {"skill", "skills", "special skill", "special skills", "power", "powers"}
    for rule in raw_band.get("band_rules") or ():
        section = str((rule.get("source") or {}).get("section", "")).casefold()
        if not any(marker in section for marker in ("skill", "power")):
            continue
