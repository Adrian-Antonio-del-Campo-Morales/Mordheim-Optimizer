...
FORMAT_VERSION = 1
FORMAT_MARKER = "MORDHEIM_WORKBOOK_V1"
DATA_SHEET = "_Mordheim"
...
def _build_summary(workbook, payload: dict) -> None:
    ...
    title = ws["A1"]
    title.value = "MORDHEIM · SIMULATION PROFILE"
...
def _build_result_sheet(workbook, result: dict, sheet_name: str) -> None:
    ...
    ws["A1"] = f"MORDHEIM · {result.get('title', 'results').upper()}"
    ws["A1"].fill = PatternFill("solid", fgColor=NAVY)
...
def load_candidate_workbook(path) -> dict:
    ...
    if DATA_SHEET not in workbook.sheetnames:
        raise CandidateWorkbookError("The workbook does not contain a Mordheim profile.")
    data = workbook[DATA_SHEET]
    if data["A1"].value != FORMAT_MARKER or data["A3"].value != FORMAT_VERSION:
        raise CandidateWorkbookError("This Mordheim workbook version is not compatible.")
    try:
